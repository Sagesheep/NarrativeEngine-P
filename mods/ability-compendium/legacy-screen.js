// Ability & Power Compendium window UI.
// This presentation layer retains the original request-shaped API, while index.js
// adapts those requests to the native modular API and owns all host integration.

/**
 * Mounts the compendium manager into a window-scoped document facade.
 * The returned cleanup only releases view-owned resources; native subscriptions
 * remain the responsibility of index.js so reopening the window cannot leak them.
 */
export default async function abilityCompendiumScreen(api, document) {
    ...
    const state = {
        ...
        characters: await api.request({ capability: 'campaign.read', resource: 'characters' }).catch(() => []),
        profile: await api.request({ capability: 'campaign.read', resource: 'character-sheet' }).catch(() => null),
        inventory: await api.request({ capability: 'campaign.read', resource: 'inventory' }).catch(() => []),
        recentPlay: await api.request({ capability: 'campaign.read', resource: 'recent-play' }).catch(() => []),
    };
    state.config = {
        schemaVersion: 2,
    ...
    /** Creates collision-resistant record IDs used by every new definition, assignment, runtime row, and proposal. */
    function uid(prefix) {
        if (globalThis.crypto && typeof globalThis.crypto.randomUUID === 'function') return globalThis.crypto.randomUUID();
        return (prefix || 'id') + '-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 10);
    }
    /** Normalizes optional form values before they enter tables or prompt text, preventing whitespace-only fields downstream. */
    function text(value) { return typeof value === 'string' ? value.trim() : ''; }
    /** Converts arrays or comma/newline-delimited form text into the canonical string-list shape used by import, rendering, and compute. */
    function list(value) {
        if (Array.isArray(value)) return value.filter((item) => typeof item === 'string').map((item) => item.trim()).filter(Boolean);
        if (typeof value === 'string') return value.split(/\r?\n|,/).map((item) => item.trim()).filter(Boolean);
        return [];
    }
    /** Parses finite numeric controls with a safe fallback so runtime and progression calculations cannot receive NaN. */
    function number(value, fallback) {
        const parsed = Number(value);
        return Number.isFinite(parsed) ? parsed : (fallback || 0);
    }
    /** Produces a comparison key for search and duplicate detection without changing the user's displayed wording. */
    function canonical(value) { return String(value || '').toLocaleLowerCase().replace(/[^\p{L}\p{N}]+/gu, ' ').trim(); }
    /** Escapes user-authored values before HTML rendering, protecting every view that interpolates imported compendium content. */
    function esc(value) {
        return String(value ?? '').replace(/[&<>"']/g, (character) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' })[character]);
    }
    /** Parses advanced JSON form fields and preserves the last valid value when an edit is incomplete or malformed. */
    function parseJson(value, fallback) {
        try { return JSON.parse(value); } catch { return fallback; }
    }
    /** Resolves a campaign terminology override while retaining the portable internal origin key used by imports and compute. */
    function originLabel(origin) { return text(state.config.terminology?.originLabels?.[origin]) || DEFAULT_ORIGIN_LABELS[origin] || origin; }
    /** Resolves a campaign terminology override while retaining the portable internal category key used by imports and compute. */
    ...
    /** Decodes a UI owner key into the owner type/id pair persisted on assignments and proposals. */
    function ownerFromKey(key) {
        const split = String(key || '').indexOf(':');
        return split > 0 ? { type: key.slice(0, split), id: key.slice(split + 1) } : null;
    }
    /** Finds a human-readable owner label for assignment cards and prompt-index ownership context. */
    function ownerName(type, id) {
        return state.characters.find((owner) => owner.type === type && owner.id === id)?.name || type + ':' + id;
    }

    /** Creates the complete default ability schema used by New, normalization, proposal acceptance, and duplication. */
    ...
    /** Validates and upgrades an imported definition into the current portable schema before it reaches the abilities table. */
    ...
    /** Creates an owner-specific ability record whose fields feed mastery, progression, variants, and prompt ownership context. */
    ...
    /** Returns or initializes the runtime row for an assignment, linking cooldowns, charges, uses, and active effects to that owner. */
    ...
    /** Updates local view state and the corresponding module table together so subsequent renders and native subscriptions remain consistent. */
    ...
    /** Shows a temporary in-window status message for save, import, discovery, and validation outcomes. */
    ...
    /** Builds authoritative but non-enforcing model context for one definition, including costs, limits, interactions, and known owners. */
    ...
    /** Rebuilds mention terms and prompt blocks after library, ownership, or runtime edits for the native prompt macro to consume. */
    ...
    document.body.innerHTML = ''
        ...
        + '</style>'
        + '<div class="shell"><div class="top"><div><div class="title">Ability &amp; Power Compendium</div><div class="muted">Installed module · campaign-owned data</div></div><div class="spacer"></div><div class="tabs" id="tabs"></div></div><div class="body" id="body"></div></div><div class="notice" id="notice"></div>';
    ...
    /** Renders primary navigation and reflects the current discovery count without rebuilding the active tab body. */
    function renderTabs() {
    ...
    /** Produces a consistently escaped input, textarea, or select used by all editor views and their collectors. */
    ...
    /** Renders searchable definitions and the full definition editor; it also reconnects the transient file-import listener after rerenders. */
    ...
    /** Converts the visible definition form into a normalized ability record that can be saved, duplicated, indexed, and exported. */
    ...
    /** Imports supported JSON compendium shapes, merges terminology, normalizes definitions, and then refreshes prompt matching data. */
    ...
    /** Serializes portable definitions plus campaign terminology and delegates the download to the native adapter. */
    async function exportCompendium() {
        ...
        await api.request({ capability: 'file.download', filename: 'ability-compendium.json', content: JSON.stringify(document, null, 2) });
        notify('Compendium exported');
    }

    /** Renders owner selection, assignments, progression controls, and runtime state for PCs and NPCs exposed by public ModData. */
    ...
    /** Merges the character assignment form into its existing row so identity and creation metadata survive edits. */
    ...
    /** Applies manual activate/advance/save/reset commands and republishes their effects to runtime and prompt-index tables. */
    ...
    /** Interprets common character-sheet lines as reviewable ability candidates without assuming or enforcing a specific ruleset. */
    ...
    /** Queues new or assignment proposals from the player sheet while suppressing already-owned, consumed, or pending entries. */
    async function importCharacterSheet() {
        const pc = state.characters.find((character) => character.type === 'pc');
        const sources = Array.isArray(state.profile?.abilities) ? state.profile.abilities : [];
        if (!pc || !sources.length) return notify('The player character sheet has no importable abilities', true);
        const byName = new Map(); state.abilities.forEach((ability) => [ability.name, ...text(ability.aliases).split(',')].forEach((name) => byName.set(canonical(name), ability)));
        const pending = new Set(state.proposals.map((proposal) => proposal.abilityId || canonical(proposal.abilityName)));
        let added = 0;
        for (const source of sources) {
            const parsed = parseSheetAbility(source); if (!parsed) continue;
            const known = byName.get(canonical(parsed.name)); const key = known?.id || canonical(parsed.name);
            if (pending.has(key) || (known && state.assignments.some((entry) => entry.ownerType === 'pc' && entry.ownerId === pc.id && entry.abilityId === known.id))) continue;
            pending.add(key); const now = Date.now();
            state.proposals.push({ id: uid('proposal'), kind: known ? 'assign' : 'new', abilityId: known?.id || '', abilityName: parsed.name, ownerType: 'pc', ownerId: pc.id, category: parsed.category, origin: parsed.origin, effect: parsed.effect, activation: parsed.activation, mastery: '', masteryTierId: '', modification: '', upgradeId: '', trainingDelta: 0, reason: 'Imported from the player character sheet for review.', evidence: source, sourceSceneId: '', sourceProfileAbility: source, createdAt: now, updatedAt: now }); added += 1;
        }
        await saveTable('proposals', state.proposals); notify(added ? 'Added ' + added + ' proposals for review' : 'All sheet abilities are already tracked or pending'); render();
    }
    /** Applies one player-approved discovery to definitions, ownership, or progression and records sheet cleanup for the compute pass. */
    ...
    /** Renders the pending-review queue with evidence and explicit individual or bulk player approval controls. */
    function renderDiscoveries() {
        document.getElementById('body').innerHTML = '<section class="card pad"><div class="toolbar section"><button class="btn primary" data-action="queue-scan">Queue AI scan</button><button class="btn" data-action="import-sheet">Import character sheet</button><button class="btn" data-action="accept-all" ' + (!state.proposals.length ? 'disabled' : '') + '>Add all</button><button class="btn danger" data-action="dismiss-all" ' + (!state.proposals.length ? 'disabled' : '') + '>Dismiss all</button><span class="muted">AI scans run safely during the next committed post-turn pass.</span></div>'
    ...
    /** Renders campaign-specific display labels while preserving stable cross-system classification keys in stored definitions. */
    function renderTerminology() {
        const originFields = ORIGINS.map((origin) => field('Origin · ' + DEFAULT_ORIGIN_LABELS[origin], 'term-origin-' + origin, state.config.terminology.originLabels?.[origin] || '')).join('');
        const categoryFields = CATEGORIES.map((category) => field('Category · ' + DEFAULT_CATEGORY_LABELS[category], 'term-category-' + category, state.config.terminology.categoryLabels?.[category] || '')).join('');
    ...
    /** Coordinates navigation rendering so every state mutation refreshes the correct manager view and its tabs. */
    function render() {
        renderTabs();
    ...
    // Central click delegation keeps one listener for all rerendered controls; each branch performs the table writes needed by dependent views.
    ...
    // Change delegation handles owner/origin selection and rebuilds only the view whose filter or ownership context changed.
    ...
    // Input delegation provides immediate library search without persisting transient query text to campaign data.
