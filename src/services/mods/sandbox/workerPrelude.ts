...
export function buildWorkerSource(modSource: string): string {
    ...
    return [
        ...
        '  throw new Error("[sandbox] " + name + " is native-tier only (EVENTS.md §5.1 / MOUNTS.md §8.1 / Phase 5.1 §2); a sandboxed compute mod cannot hold a listener, a mount closure, or a macro resolver across a run");',
        '};',
