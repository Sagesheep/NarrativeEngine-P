// FROZEN FIXTURE — the pre-extraction enemy validator. DO NOT "FIX" THIS FILE.
//
// This is `server/lib/enemySchema.js` exactly as it stood at commit 2f568cd,
// the last commit before the enemy subsystem left core. It is the 443 lines of
// defensive coercion that every existing campaign's data has been passing
// through for the life of the app — which means real saves on real disks
// almost certainly contain records that are only well-formed BECAUSE of it
// (Phase 8.5 §3.5).
//
// It lives here so `enemyValidatorPort.test.js` can prove that the copy now
// running inside the `enemies` mod repairs data the same way. That proof is
// the whole basis for Phase 8.5 §3.5 and Phase 8.2 §9's stop condition: a
// validator that drops a field on data that survives today is silent data loss
// wearing a refactor's clothes.
//
// If the mod's validator changes deliberately, the TEST records the divergence
// with a reason. This file never moves. It is a historical record, and editing
// it to make a test pass destroys the only evidence of what the app used to do.
//
// Two edits were made on vendoring, both mechanical, both marked at the line:
//   1. the shared-shape import was inlined, because
//      `@narrative/engine/enemy/enemyShape` was deleted by the same phase
//      (ENEMY_SEAM flag #5). The values below are verbatim from that file at
//      the same commit;
//   2. `randomUUID` comes from the Web Crypto global instead of `node:crypto`
//      — the same function in Node, and the one the mod's copy calls, so the
//      differential test can stub one id generator over both.
// No validation logic was touched.

// Vendoring edit #2 (see the header): `import { randomUUID } from 'node:crypto'`
// became the Web Crypto global, which in Node is the same function. The mod's
// copy calls the global because it runs in a browser, and the differential test
// pins one stub over both so a generated id is compared rather than skipped.
